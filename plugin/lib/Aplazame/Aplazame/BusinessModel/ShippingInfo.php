...
class Aplazame_Aplazame_BusinessModel_ShippingInfo {
	...
	public static function createFromOrder( WC_Order $order ) {
	    ...
	    $shippingInfo->first_name = WC_Aplazame::_m_or_a( $order, 'get_shipping_first_name', 'shipping_first_name' );
	    $shippingInfo->last_name = WC_Aplazame::_m_or_a( $order, 'get_shipping_last_name', 'shipping_last_name' );
	    $shippingInfo->street = WC_Aplazame::_m_or_a( $order, 'get_shipping_address_1', 'shipping_address_1' );
	    ...
	    $shippingInfo->address_addition = WC_Aplazame::_m_or_a( $order, 'get_shipping_address_2', 'shipping_address_2' );
	    ...
	    if ( $total_shipping > 0 ) {
	    	...
	    	$shippingInfo->tax_rate = Aplazame_Sdk_Serializer_Decimal::fromFloat( 100 * $shipping_tax / $total_shipping );
		}
